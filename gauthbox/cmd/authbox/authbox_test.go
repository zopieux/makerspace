...
func TestCombinedOnOffButton(t *testing.T) {
	// Restore hardware mocks after test
	oldRequestOutputPinFn := gauthbox.RequestOutputPinFn
	oldRequestInputPinFn := gauthbox.RequestInputPinFn
	oldWriteSysfsLedFn := gauthbox.WriteSysfsLedFn
	oldWriteSysfsLedTriggerFn := gauthbox.WriteSysfsLedTriggerFn
	oldSdNotifyFn := gauthbox.SdNotifyFn
	oldBadgeReaderFn := gauthbox.BadgeReaderFn
	t.Cleanup(func() {
		gauthbox.RequestOutputPinFn = oldRequestOutputPinFn
		gauthbox.RequestInputPinFn = oldRequestInputPinFn
		gauthbox.WriteSysfsLedFn = oldWriteSysfsLedFn
		gauthbox.WriteSysfsLedTriggerFn = oldWriteSysfsLedTriggerFn
		gauthbox.SdNotifyFn = oldSdNotifyFn
		gauthbox.BadgeReaderFn = oldBadgeReaderFn
	})
	...
	assertMQTTMessages := func(expected map[string]string, timeout time.Duration) {
		...
		for len(expected) > 0 {
			...
			case msg := <-mqttBroker.PubChan:
				...
				// Also check if this is the Home Assistant discovery config topic
				if msg.Topic == "homeassistant/device/"+haDeviceId+"/config" {
					var cfg map[string]interface{}
					if err := json.Unmarshal([]byte(msg.Payload), &cfg); err == nil {
						if dev, ok := cfg["device"].(map[string]interface{}); ok {
							if model, ok := dev["model"].(string); ok {
								lowerModel := strings.ToLower(model)
								if !strings.Contains(lowerModel, "on-off") && !strings.Contains(lowerModel, "onoff") {
									t.Errorf("Expected model to contain 'on-off' or 'onoff', got: %s", model)
								}
							} else {
								t.Errorf("model is not a string in discovery config")
							}
						}
					}
				}
	...
	assertMQTTMessages(expectedMQTTMsgMap(hostname, "onoffbutton_topic"), time.Second)
	...
	// Also verify that there's a status notification containing "device_type: onoff"
	var foundStatusNotification bool
	for len(sdNotifyChan) > 0 {
		n := <-sdNotifyChan
		if strings.HasPrefix(n, "STATUS=") && strings.Contains(n, "device_type: onoff") {
			foundStatusNotification = true
		}
	}
	if !foundStatusNotification {
	...
	if !foundStatusNotification {
		t.Errorf("Did not find Systemd STATUS notification containing 'device_type: onoff'")
	}
...
func TestCombinedButtonless(t *testing.T) {
	// Restore hardware mocks after test
	oldRequestOutputPinFn := gauthbox.RequestOutputPinFn
	oldRequestInputPinFn := gauthbox.RequestInputPinFn
	oldWriteSysfsLedFn := gauthbox.WriteSysfsLedFn
				...
				if msg.Topic == "homeassistant/device/"+haDeviceId+"/config" {
					...
					if err := json.Unmarshal([]byte(msg.Payload), &cfg); err == nil {
						if dev, ok := cfg["device"].(map[string]interface{}); ok {
							if model, ok := dev["model"].(string); ok {
								lowerModel := strings.ToLower(model)
								if !strings.Contains(lowerModel, "button-less") && !strings.Contains(lowerModel, "buttonless") {
									t.Errorf("Expected model to contain 'button-less' or 'buttonless', got: %s", model)
								}
							} else {
								t.Errorf("model is not a string in discovery config")
							}
	...
	// Verify MQTT initial discovery and state
	assertMQTTMessages(expectedMQTTMsgMap(hostname, "buttonless_topic"), time.Second)
	...
	// Also verify that there's a status notification containing "device_type: buttonless"
	var foundStatusNotification bool
	for len(sdNotifyChan) > 0 {
		n := <-sdNotifyChan
		if strings.HasPrefix(n, "STATUS=") && strings.Contains(n, "device_type: buttonless") {
			foundStatusNotification = true
		}
	}
	if !foundStatusNotification {
	...
	if !foundStatusNotification {
		t.Errorf("Did not find Systemd STATUS notification containing 'device_type: buttonless'")
	}
...
func expectedMQTTMsgMap(hostname string, topic string) map[string]string {
	haDeviceId := "authbox_" + hostname
	return map[string]string{
		"homeassistant/device/" + haDeviceId + "/config": "*",
		topic + "/" + haDeviceId + "/LWT":                "*",
	}
}
